...
**https://hjgantt.vercel.app** — 팀원에게 이 주소만 주면 각자 올리고 각자 받습니다.

`main`에 푸시하면 Vercel이 자동 배포합니다. 로컬에서 직접 올리려면 `vercel --prod`.
진입점은 `api/index.py` 6줄이고 루트의 `app.py`를 그대로 불러옵니다 — 로컬
(`./run.sh`)과 배포본이 **같은 코드**를 돌게 하기 위해서입니다.
...
**업로드 파일은 서버에 저장되지 않습니다.** 요청 전용 임시 폴더에서 만들고,
결과 엑셀은 응답에 실려 브라우저가 파일로 저장합니다. 요청이 끝나면 폴더째
사라집니다. 양식도 서버에 기억시키지 않습니다(올리면 그 요청 한 번만 적용).

**예외는 실행 이력 하나뿐입니다.** "지난번 대비 무엇이 바뀌었나"를 답하려면
기준선이 필요해서, 비교용 최소 정보(제품·과제명·유형·상태·날짜 4개)만
Upstash Redis에 최근 10회분 남깁니다. 담당자명은 저장하지 않습니다.
저장소 환경변수가 없으면 이력 기능만 꺼진 채 나머지는 그대로 동작합니다.
